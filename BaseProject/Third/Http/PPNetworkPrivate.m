...
void PPLog(NSString *format, ...) {
#ifdef DEBUG
    if (![PPNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        PPLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
+ (NSStringEncoding)stringEncodingWithRequest:(PPBaseRequest *)request {
...
@implementation PPBaseRequest (RequestObserver)

- (void)tellObserversRequestWillStart {
    for (id<PPBaseRequestObserver> observer in self.observersArray) {
...
@end
//
@implementation PPBatchRequest (RequestObserver)
...
@implementation PPChainRequest (RequestObserver)
